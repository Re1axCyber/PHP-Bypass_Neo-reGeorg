...
import json
def padZero(data, block_size=32):
    """
    Pad the input data with zeros to the specified block size.
    """
    return data + (int(block_size) - len(data) % int(block_size)) * b'\0'
# AES加密
def aes_encrypt(data, key, iv):
    """
    AES-128-CBC encryption using the specified key and initialization vector.
    """
    cipher = AES.new(key, AES.MODE_CBC, iv)
    return base64.b64encode(cipher.encrypt(padZero(data)))
# AES解密
def aes_decrypt(encoded_data, key, iv):
    """
    AES-128-CBC decryption using the specified key and initialization vector.
    """
    decoded_data = base64.b64decode(encoded_data)
    cipher = AES.new(key, AES.MODE_CBC, iv)
    plain_text=cipher.decrypt(decoded_data).decode().replace('\x00', '')
    return plain_text

# AES密钥及偏移量
key = b'qianxinnb'
iv = b'ffffuuuucccckkkk'
...
#HOSTFAIL         = b"\x04"
#TTLEXPIRED       = b"\x06"
#UNSUPPPAuth_key     = b"\x07"
#ADDRTYPEUNSPPP = b"\x08"
#UNASSIGNED       = b"\x09"
...
def blv_encode(info):
    ...
    data = struct.pack('>bi{len}s'.format(len=head_len), 12, head_len + BLV_L_OFFSET, head_rand)  # 头部填充
    ...
    data += struct.pack('>bi{len}s'.format(len=tail_len), 21, tail_len + BLV_L_OFFSET, tail_rand)  # 尾部填充
    return data
...
def encode_body(info):
    datas = blv_encode(info)
    data = base64.b64encode(datas)
    if ispython3:
        data = data.decode()
...
class SocksAuth_keyNotImplemented(Exception):
    pass
...
class Rand:
    def __init__(self, key):
        salt = b'11f271c6sb360ypkptad1uv6e1ut1fu0pt4xillz1w9bbs2shabi89z9gca9dcaonimauvgjfr331o0szln'
        key_min_len = 28
        # 密码强度不足时，使用加盐hash
...
class session(Thread):
    ...
    def parseSocks5(self, sock):
        ...
        if ver == b"\x02":                # this is a hack for proxychains
            ver, Auth_key, rsv, atyp = (sock.recv(1), sock.recv(1), sock.recv(1), sock.recv(1))
        else:
            Auth_key, rsv, atyp = (sock.recv(1), sock.recv(1), sock.recv(1))
        target = None
        targetPort = None
        if atyp == b"\x01":      # TTv4
            target = sock.recv(4)
        ...
        if Auth_key == b"\x02":   # BIND
            raise SocksAuth_keyNotImplemented("Socks5 - BIND not implemented")
        elif Auth_key == b"\x03": # UDP
            raise SocksAuth_keyNotImplemented("Socks5 - UDP not implemented")
        elif Auth_key == b"\x01": # CONNECT
            try:
        ...
        raise SocksAuth_keyNotImplemented("Socks5 - Unknown Auth_key")
    ...
    def handleFwd(self, sock):
        log.debug("[PP FWD] Forward detected")
    ...
    def neoreg_request(self, info, timeout=None):
        ...
        string=json.dumps(encode_body(info)).encode('utf-8')
        data=aes_encrypt(string, key, iv)
        log.debug("[HTTP] [%s:%d] %s Request (%s)" % (self.target, self.port, info['Auth_key'], self.mark))
        # print(datas)
        retry = 0
        # print(HEADERS)
        while True:
            ...
            try:
                ...
                headers.update({'Authorzions': base64.b64encode(data)})
                # print(headers)
                response = self.conn.get(self.url_sample(), headers=headers, timeout=timeout)
                second = response.elapsed.total_seconds()
                log.debug("[HTTP] [%s:%d] %s Response (%s) => HttpCode: %d, Time: %.2fs" % (self.target, self.port, info['Auth_key'], self.mark, response.status_code, second))
                rdata = response.headers['Auth_Response']
                rdata = rdata.encode('utf-8')
                decoded_data = aes_decrypt(rdata, key, iv).encode('utf-8')
                rinfo = decode_body(decoded_data)
                if rinfo is None:
                    raise NeoregReponseFormatError("[HTTP] Response Format Error: {}".format(response.content))
                else:
                    if rinfo['STATUS'] != 'OK' and info['Auth_key'] != 'DISCONNECT':
                        log.warning('[%s] [%s:%d] Error: %s' % (info['Auth_key'], self.target, self.port, rinfo['ERROR']))
                    return rinfo

                # 高并发下，csharp 容易出现 503, 重试即可
                log.warning("[HTTP] [%s:%d] [ReTry %d] %s Request (%s) => HttpCode: %d" % (self.target, self.port, retry, info['Auth_key'], self.mark, response.status_code))

            except requests.exceptions.ConnectionError as e:
                log.warning('[HTTP] [{}] [requests.exceptions.ConnectionError] {}'.format(info['Auth_key'], e))
            except requests.exceptions.ChunkedEncodingError as e: # python2 requests error
                log.warning('[HTTP] [{}] [requests.exceptions.ChunkedEncodingError] {}'.format(info['Auth_key'], e))
            except NeoregReponseFormatError as e: # python2 requests error
                log.warning("[%s] [%s:%d] NeoregReponseFormatError, Retry: No.%d" % (info['Auth_key'], self.target, self.port, retry))
                if retry > MAXRETRY:
                    raise e
    ...
    def setupRemoteSession(self, target, port):
        ...
        info = {'Auth_key': 'CONNECT', 'MARK': self.mark, 'TT': self.target, 'PP': str(self.port)}
    ...
    def closeRemoteSession(self):
        if not self.connect_closed:
            ...
            if hasattr(self, 'mark'):
                info = {'Auth_key': 'DISCONNECT', 'MARK': self.mark}
                rinfo = self.neoreg_request(info)
            if not self.connect_closed:
                if hasattr(self, 'target'):
    ...
    def reader(self):
        try:
            info = {'Auth_key': 'READ', 'MARK': self.mark}
            n = 0
            while True:
                try:
    ...
    def writer(self):
        try:
            info = {'Auth_key': 'FORWARD', 'MARK': self.mark}
            n = 0
            while True:
                try:
    ...
    def run(self):
        ...
        except SocksAuth_keyNotImplemented as ex:
            log.error('[SOCKS5] [SocksAuth_keyNotImplemented] {}'.format(ex))
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        parser.add_argument("-t", "--target", metavar="TT:PP", help="Network forwarding Target, After setting this parameter, port forwarding will be enabled")
        parser.add_argument("-k", "--key", metavar="KEY", required=True, help="Specify connection key")
        parser.add_argument("-l", "--listen-on", metavar="TT", help="The default listening address (default: 127.0.0.1)", default="127.0.0.1")
        parser.add_argument("-p", "--listen-port", metavar="PP", help="The default listening port (default: 1080)", type=int, default=1080)
    ...
    if 'url' in args:
        ...
        for filename in os.listdir(script_dir):
            ...
            if os.path.isfile(filepath) and filename.startswith('tunnel.'):
                ...
                text = text.replace(r"Qianxin, 'Niubi'", neoreg_hello)
